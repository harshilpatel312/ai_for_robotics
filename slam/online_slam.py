...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        ## expand the matrix to accomodate the position at t+1
        expansion_list = [i for i in range(dim+2)]
        # delete elements at idxs 2 and 3, corresponding to x(t+1) and y(t+1)
        del expansion_list[2]
        del expansion_list[2] # 2 because list mutated earlier
        omega = omega.expand(dim+2, dim+2, expansion_list, expansion_list)
        Xi = Xi.expand(1, dim+2, [0], expansion_list)

        ## motion constraints
        omega.value[0][0] += 1/motion_noise # x(i)
        omega.value[0][2] -= 1/motion_noise # x(i+1)
        omega.value[2][0] -= 1/motion_noise # x(i)
        omega.value[2][2] += 1/motion_noise # x(i+1)

        omega.value[1][1] += 1/motion_noise # y(i)
        omega.value[1][3] -= 1/motion_noise # y2(i+1)
        omega.value[3][1] -= 1/motion_noise # y(i)
        omega.value[3][3] += 1/motion_noise # y2(i+1)

        Xi.value[0][0] -= motion[0]/motion_noise # x(i)
        Xi.value[0][2] += motion[0]/motion_noise # x(i+1)

        Xi.value[0][1] -= motion[1]/motion_noise # y(i)
        Xi.value[0][3] += motion[1]/motion_noise # y(i+1)

        ## measurement constraints
        for measure in measurement:
            idx = 2 * (2 + measure[0])
            omega.value[0][0] += 1/measurement_noise # x(i)
            omega.value[0][idx] -= 1/measurement_noise # L
            omega.value[idx][0] -= 1/measurement_noise # x(i)
            omega.value[idx][idx] += 1/measurement_noise # L

            omega.value[1][1] += 1/measurement_noise # y(i)
            omega.value[1][idx + 1] -= 1/measurement_noise # L
            omega.value[idx+1][1] -= 1/measurement_noise # y(i)
            omega.value[idx+1][idx+1] += 1/measurement_noise # L

            Xi.value[0][0] -= measure[1]/measurement_noise # x(i)
            Xi.value[0][idx] += measure[1]/measurement_noise # L

            Xi.value[0][1] -= measure[2]/measurement_noise # y(i)
            Xi.value[0][idx+1] += measure[2]/measurement_noise  # L

        ## factor out the previous pose
        omega_dash_take = [int(x) for x in list(np.linspace(2, omega.dimx - 1, omega.dimx - 2))]
        omega_dash = omega.take(omega_dash_take, omega_dash_take)

        B = omega.take([0,1], [0,1])

        A_take = [int(x) for x in list(np.linspace(2, omega.dimx - 1, omega.dimx - 2))]
        A = omega.take([0,1], A_take)

        Xi_dash_take = [int(x) for x in list(np.linspace(2, dim+1, dim))]
        Xi_dash = Xi.take([0], Xi_dash_take)
        C = Xi.take([0], [0,1])

        omega = omega_dash - A.transpose() * B.inverse() * A
        Xi = Xi_dash.transpose() - A.transpose() * B.inverse() * C.transpose() # changed formula because dimensions weren't matching up
        Xi = Xi.transpose() # to correct for the changed formula (revert back to original form of Xi)
