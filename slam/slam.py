...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # motion constraints
        omega.value[n][n] += 1 # x(i)
        omega.value[n][n+2] -= 1 # x(i+1)
        omega.value[n+2][n] -= 1 # x(i)
        omega.value[n+2][n+2] += 1 # x(i+1)

        omega.value[n+1][n+1] += 1 # y(i)
        omega.value[n+1][n+3] -= 1 # y2(i+1)
        omega.value[n+3][n+1] -= 1 # y(i)
        omega.value[n+3][n+3] += 1 # y2(i+1)

        Xi.value[0][n] -= motion[0] # x(i)
        Xi.value[0][n+2] += motion[0] # x(i+1)

        Xi.value[0][n+1] -= motion[1] # y(i)
        Xi.value[0][n+3] += motion[1] # y(i+1)

        # measurement constraints
        for measure in measurement:
            idx = 2 * (N + measure[0])
            omega.value[n][n] += 1 # x(i)
            omega.value[n][idx] -= 1 # L
            omega.value[idx][n] -= 1 # x(i)
            omega.value[idx][idx] += 1 # L

            omega.value[n+1][n+1] += 1 # y(i)
            omega.value[n+1][idx + 1] -= 1 # L
            omega.value[idx+1][n+1] -= 1 # y(i)
            omega.value[idx+1][idx+1] += 1 # L

            Xi.value[0][n] -= measure[1] # x(i)
            Xi.value[0][idx] += measure[1] # L

            Xi.value[0][n+1] -= measure[2] # y(i)
            Xi.value[0][idx+1] += measure[2]  # L
