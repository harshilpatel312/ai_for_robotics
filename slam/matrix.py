...
# list_for_expansion = [i for i in range(dim+2)]
# del list_for_expansion[2]
# del list_for_expansion[2]
#
# M = M.expand(dim + 2, dim + 2, list_for_expansion, list_for_expansion)
# M.value[2][2] = 100
# M.value[3][3] = 100
#
# M.show()
#
# omega_dash_take = [int(x) for x in list(np.linspace(2, dim+1, dim))]
# omega_dash = M.take(omega_dash_take, omega_dash_take)
# omega_dash.show()
#
# B = M.take([0,1], [0,1])
# B.show()
#
# A_take = [int(x) for x in list(np.linspace(2, dim+1, dim))]
# A = M.take([0,1], A_take)
# A.show()
#
# print('\n===================\n')
...
Xi = Xi.expand(1, dim+2, [0], [0,1,4,5,6,7,8,9,10,11])
Xi.show()

# Xi_dash_take = [int(x) for x in list(np.linspace(2, 9, 8))]
# # print(Xi_dash_take)
